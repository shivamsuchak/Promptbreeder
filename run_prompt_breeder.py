...
#         # response = openai.chat.completions.create(model="gpt-3.5-turbo", messages=[{"role": "user", "content": prompt}], max_tokens=600)
#         # unit.P = response.choices[0].text.strip()
#         response = openai.chat.completions.create(model="gpt-3.5-turbo", messages=[{"role": "system", "content": f"You are a {unit.M} assistant."}, {"role": "user", "content": f"Solve the {unit.T} problem: {population.problem_description}"}])
...
#     openai.api_key = "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"
